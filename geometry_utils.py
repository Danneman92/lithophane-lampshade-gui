...
def stitch_rings(indices: List[List[int]], inner_idx: List[int], outer_idx: List[int], outward: bool = True) -> None:
    m = len(inner_idx)
    for j in range(m):
        jn = (j + 1) % m
...
def stitch_wall(vertices, colors, normals, indices, base_xyz, tip_xyz, color=(0.6,0.6,0.6), up=True) -> None:
    start_base = len(vertices)
    for x,y,z in base_xyz:
        vertices.append([x,y,z]); colors.append(list(color)); normals.append([0, 1 if up else -1, 0])
...
def build_ring_xyz_at_radius(radius: float, y: float, ncols: int, num_panels: int, panels_present: List[bool]) -> List[List[float]]:
    ring = []
