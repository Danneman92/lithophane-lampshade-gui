...
def stitch_rings(indices: List[List[int]],
    ...
    """Stitch two same-length rings into a quad strip."""
    m = len(inner_idx)
    for j in range(m):
        jn = (j + 1) % m
...
def build_ring_xyz_at_radius(radius: float, y: float,
    ...
    """Full-circle ring — always all panels for a gap-free ring."""
    ring = []
